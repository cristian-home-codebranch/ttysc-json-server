...
\`\`\``;
  ...
  // Endpoint de chat con respuesta en streaming (SSE) - MOCK LOCAL
  server.post("/chat/stream", async (req, res) => {
    try {
      // Generar stream SSE de respuesta en chunks
      const db = router.db;
      const { chatId, messages } = req.body || {};
      ...
      if (!lastUserMessage) {
        res
          .status(400)
          .json({ success: false, error: "No user message found" });
        return;
      }
      ...
      // Comentario inicial para activar el stream en algunos proxies/clients
      res.write(`: keep-alive\n\n`);

      // Generar contenido mock (reutiliza pauta de /chat pero en stream)
      const shouldIncludeCode =
        Math.random() < 0.1 ||
        lastUserMessage.content?.toLowerCase().includes("code");
      const shouldIncludeTable =
        Math.random() < 0.1 ||
        lastUserMessage.content?.toLowerCase().includes("table");
      const shouldIncludeChart =
        Math.random() < 0.1 ||
        lastUserMessage.content?.toLowerCase().includes("chart");
      ...
      if (SHOW_MARKDOWN && shouldIncludeCode)
        responseContent += `\n\n${codeContent}`;
      if (SHOW_MARKDOWN && shouldIncludeTable)
        responseContent += `\n\n${tableContent}`;
      if (SHOW_MARKDOWN && shouldIncludeChart)
        responseContent += `\n\n${chartContent}`;

      // Funciones auxiliares
      const toChunks = (text, size) => {
        const out = [];
      ...
      const assistantMessageId = uuidv6();
      const allChunks = toChunks(responseContent, 80);
      let aggregated = "";

      // Enviar evento de apertura (sin payload de rol)
      ...
      let idx = 0;
      const delayMs = parseInt(process.env.CHAT_STREAM_DELAY_MS || "250", 10);
      const sendPiece = () => {
        try {
          if (idx >= allChunks.length) {
            ...
            // Persistir el mensaje del asistente
            const botResponse = {
              id: assistantMessageId,
              role: "assistant",
              title: "Response from bot (stream)",
              content: aggregated,
              created: new Date().toISOString(),
            };
            const latestChats = db.get("chats").value();
            const idxChat = latestChats.findIndex((c) => c.id === chatId);
            if (idxChat !== -1) {
              latestChats[idxChat].messages =
                latestChats[idxChat].messages || [];
              latestChats[idxChat].messages.push(botResponse);
              db.set("chats", latestChats).write();
            }
            try {
              res.end();
            } catch {}
          ...
          if (!ok) {
            res.once("drain", () => setTimeout(sendPiece, delayMs));
          } else {
            setTimeout(sendPiece, delayMs);
          }
        } catch (e) {
          console.error("SSE sendPiece error:", e);
          try {
            res.end();
          } catch {}
        }
