...
const { SUPPLY_CHAIN_MARKDOWN } = require("./supply_chain_markdown");
const fs = require("fs");
...
function createServer(dbPath) {
  ...
  const universalWrapper = (req, res, next) => {
    // Omitir completamente el envoltorio para el endpoint de streaming
    // y cualquier otra ruta POST de SSE que necesite datos crudos.
  ...
  // Middleware para loggear claramente los encabezados de cada petición entrante
  server.use((req, res, next) => {
    try {
      console.log(
        `\n➡️ Incoming request - ${req.method} ${
          req.url
        } - ${new Date().toISOString()}`
      );
      // Iterar y mostrar cada header en formato legible
      Object.keys(req.headers).forEach((name) => {
        const value = req.headers[name];
        if (name.toLowerCase() === "authorization") {
          // No imprimir la credencial completa por seguridad; mostrar solo el esquema
          if (typeof value === "string") {
            const scheme = value.split(" ")[0];
            console.log(`  ${name}: ${scheme} [REDACTED]`);
          } else {
      ...
      console.log("Error logging headers:", err.message);
    }
    next();
  });
  ...
  server.post("/chat", (req, res) => {
    ...
    const shouldIncludeTable =
      Math.random() < 0.1 ||
      lastUserMessage.content.toLowerCase().includes("table"); // 10% de probabilidad o si contiene "table"
    const shouldIncludeChart =
      Math.random() < 0.1 ||
      lastUserMessage.content.toLowerCase().includes("chart"); // 10% de probabilidad o si contiene "chart"

    const shouldIncludeMarkdown = lastUserMessage.content
      .toLowerCase()
      .includes("markdown");
...
\`\`\``;
    ...
    const chartContent = `[SHOW_CHART:${uuidv6()}]`;

    let responseContent =
      "LATAM OTIF dropped to 88.4% due to supplier constraints and lane delays. Actions: 1) Air-freight rails (cap ≤ $12k), 2) Rebalance LIM-02 → BOG-01, 3) Temporary carrier swap. Monitor OTIF daily; escalate if <90% for 3 days.";
  ...
  // Endpoint de chat con respuesta en streaming basado en archivo db/chats.json
  // Requerido payload:
  // {
  ...
  server.post("/chat/stream", async (req, res) => {
    try {
      const { messages, model, temperature, max_tokens, top_p } =
        req.body || {};
      ...
      if (!Array.isArray(messages) || messages.length === 0)
        errors.push(
          "messages (array) is required and must have at least 1 item"
        );
      if (Array.isArray(messages)) {
        ...
        if (
          ...
          errors.push(
            "first messages[0] must have role=user and non-empty content"
          );
      }
      if (typeof model !== "string" || !model.trim())
        errors.push("model (string) is required");
      if (typeof temperature !== "number")
        errors.push("temperature (number) is required");
      if (typeof max_tokens !== "number")
        errors.push("max_tokens (number) is required");
      if (typeof top_p !== "number") errors.push("top_p (number) is required");
      ...
      // Leer archivo chats.json (contiene los chunks a emitir)
      const chatsFilePath = path.join(__dirname, "..", "db", "chats.json");
      let fileRaw;
      try {
        fileRaw = await fs.promises.readFile(chatsFilePath, "utf-8");
      } catch (e) {
        console.error("Error reading chats.json:", e);
        return res
          .status(500)
          .json({ success: false, error: "Cannot read chats.json" });
      }

      let chunks;
      try {
        chunks = JSON.parse(fileRaw);
        if (!Array.isArray(chunks)) throw new Error("File is not an array");
      } catch (e) {
        console.error("Error parsing chats.json:", e);
        return res
          .status(500)
          .json({ success: false, error: "Invalid chats.json format" });
      }

      const delayMs = parseInt(process.env.CHATS_STREAM_DELAY_MS || "250", 10);
      let index = 0;
      ...
      const sendNext = () => {
        try {
          ...
          const obj = chunks[index++];
          // Enviar chunk tal cual está en el archivo
          ...
          console.error("Stream sendNext error:", err);
          try {
            res.end();
          } catch {}
        }
      ...
      // Heartbeat para mantener conexión abierta en proxies
  ...
  // Auxiliary endpoints
  const chartExamples = require("./chartExamples");
  const tableExamples = require("./tableExamples");

  // Endpoint único para charts simplificado
  server.get("/auxiliary/chart/:chartId", (req, res) => {
    try {
      const db = router.db;
      const { chartId } = req.params;
      const { from, to, region, dateFrom, dateTo } = req.query;
      const startDateParam = dateFrom || from;
      const endDateParam = dateTo || to;

      const chartMappings = db.get("chartMappings").value() || [];
      let mapping = chartMappings.find((m) => m.uuid === chartId);

      if (!mapping) {
        const randomChart =
          chartExamples[Math.floor(Math.random() * chartExamples.length)];
        mapping = {
          uuid: chartId,
          chartExampleId: randomChart.id,
          createdAt: new Date().toISOString(),
        };
        db.set("chartMappings", [...chartMappings, mapping]).write();
      }

      const baseChart = chartExamples.find(
        (c) => c.id === mapping.chartExampleId
      );
      const needsFilter = Boolean(startDateParam || endDateParam || region);
      const filteredChart = needsFilter
        ? JSON.parse(JSON.stringify(baseChart))
        : { ...baseChart };

      // Helpers para manipulación de charts
      const MONTHS = [
        "Jan",
        "Feb",
        "Mar",
      ...
      const isMonthlyChart =
        Array.isArray(filteredChart.chart?.labels) &&
        filteredChart.chart.labels.length <= 12 &&
        filteredChart.chart.labels.every((l) => MONTHS.includes(l));
      const scaleArrayNumbers = (arr, factor) =>
        arr.map((v) => {
          if (typeof v !== "number") return v;
          return Math.round(v * factor);
        });
      const scaleSeriesObjects = (seriesArr, factor) =>
        seriesArr.map((s) => ({
          ...s,
          data: Array.isArray(s.data)
            ? scaleArrayNumbers(s.data, factor)
            : s.data,
        }));
      const sliceMonthly = (labels, startIndex, endIndex) =>
        labels.slice(startIndex, endIndex + 1);

      const applyFactorToChartData = (chart, factor) => {
        if (!Array.isArray(chart.chart?.data)) return chart;
        const isSeriesWithObjects =
          chart.chart.data.length > 0 &&
          typeof chart.chart.data[0] === "object" &&
          Array.isArray(chart.chart.data[0].data);
        chart.chart.data = isSeriesWithObjects
          ? scaleSeriesObjects(chart.chart.data, factor)
          : scaleArrayNumbers(chart.chart.data, factor);
        return chart;
      };

      const applyMonthlyRange = (chart, startIndex, endIndex) => {
        if (Array.isArray(chart.chart?.labels)) {
          chart.chart.labels = sliceMonthly(
            chart.chart.labels,
            startIndex,
            endIndex
          );
        ...
        if (Array.isArray(chart.chart?.data)) {
          const isSeriesWithObjects =
            chart.chart.data.length > 0 &&
            typeof chart.chart.data[0] === "object" &&
            Array.isArray(chart.chart.data[0].data);
          if (isSeriesWithObjects) {
            chart.chart.data = chart.chart.data.map((series) => ({
              ...series,
              data: series.data.slice(startIndex, endIndex + 1),
            }));
          } else {
            chart.chart.data = chart.chart.data.slice(startIndex, endIndex + 1);
          }
        }
        return chart;
      };
      ...
      if (startDateParam || endDateParam) {
        // Si el chart es mensual, devolver SOLO los meses elegidos
        if (isMonthlyChart) {
          const fromDate = new Date(startDateParam || "2024-01-01");
          const toDate = new Date(endDateParam || "2024-12-31");
          const startMonth = fromDate.getMonth();
          const endMonth = toDate.getMonth();

          const startIndex = Math.max(0, Math.min(11, startMonth));
          const endIndex = Math.max(startIndex, Math.min(11, endMonth));

          applyMonthlyRange(filteredChart, startIndex, endIndex);
        } else {
          // Cuando NO hay eje tiempo mensual, solo simular y disminuir
          const decreaseFactor = 0.9; // -10%
          applyFactorToChartData(filteredChart, decreaseFactor);
        }
      } else {
        // Sin filtros de fecha: simular disminución general
        const decreaseFactor = 0.9; // -10%
        applyFactorToChartData(filteredChart, decreaseFactor);
      }

      if (region) {
        // Región: solo disminuir cantidades
        const factors = {
          north: 0.97,
          south: 0.9,
          east: 0.95,
        ...
        const factor = factors[region.toLowerCase()] ?? 0.95;

        applyFactorToChartData(filteredChart, factor);
      }
      ...
      console.error("Error en endpoint /auxiliary/chart:", error);
      res
        .status(500)
        .json({ success: false, error: "Error generando datos del chart" });
    }
  });

  // Endpoint único para tables con mapeo persistente y filtros estándar
  server.get("/auxiliary/table/:tableId", (req, res) => {
    try {
      const db = router.db;
      const { tableId } = req.params;
      const { dateFrom, dateTo } = req.query;

      const tableMappings = db.get("tableMappings").value() || [];
      let mapping = tableMappings.find((m) => m.uuid === tableId);

      if (!mapping) {
        const randomTable =
          tableExamples[Math.floor(Math.random() * tableExamples.length)];
        mapping = {
          uuid: tableId,
          tableExampleId: randomTable.id,
          createdAt: new Date().toISOString(),
        };
        db.set("tableMappings", [...tableMappings, mapping]).write();
      }

      const baseTable = tableExamples.find(
        (t) => t.id === mapping.tableExampleId
      );
      const filteredPayload = JSON.parse(JSON.stringify(baseTable));

      // Filtro de rango por fechas en campo created_date
      if (dateFrom || dateTo) {
        filteredPayload.table.rows = filteredPayload.table.rows.filter(
          (row) => {
            const rowDate = new Date(row.created_date);
            let matches = true;
            if (dateFrom) matches = matches && rowDate >= new Date(dateFrom);
            if (dateTo) matches = matches && rowDate <= new Date(dateTo);
            return matches;
          }
        );
      }

      // Aplicar filtros, orden y paginación tipo JSON Server
      const queryForRows = { ...req.query };
      delete queryForRows.dateFrom;
      delete queryForRows.dateTo;

      let rows = filteredPayload.table.rows;
      rows = applyJsonServerFilters(rows, queryForRows);
      rows = applySorting(rows, queryForRows);
      rows = applyPagination(rows, queryForRows, res);
      filteredPayload.table.rows = rows;
      ...
      console.error("Error en endpoint /auxiliary/table:", error);
      res
        .status(500)
        .json({ success: false, error: "Error generando datos de la tabla" });
    }
  });

  // Usar el router por defecto para otras rutas
  // Endpoint para recibir métricas de usuario
  server.post("/api/v1/usermetric", (req, res) => {
    try {
      const payload = req.body || {};

      // Validaciones básicas según la interfaz requerida
      const requiredFields = [
        "ConversationId",
        "WorkflowName",
        "SessionId",
        "IsRecordableConversation",
        "Query",
        "Response",
        "Source",
        "UserBrowser",
      ...
      // Validate Environment and Source strictly
      const allowedEnvs = ["dev", "stg", "prd"];
      if (!allowedEnvs.includes(payload.Environment)) {
        return res.status(400).json({
          success: false,
      ...
      if (payload.Source !== "TTYSC") {
        return res
          .status(400)
          .json({ success: false, error: "Invalid Source. Must be 'TTYSC'" });
      }

      // Construir el objeto que sí vamos a almacenar (ignorar campos NO utilizados)
      const metric = {
        id: uuidv6(),
        conversationId: payload.ConversationId,
        workflowName: payload.WorkflowName,
        sessionId: payload.SessionId,
        isRecordableConversation: Boolean(payload.IsRecordableConversation),
        query: payload.Query,
        response: payload.Response,
        // Forzar Source a TTYSC (trusted)
        source: "TTYSC",
        userBrowser: payload.UserBrowser,
        // Campos opcionales permitidos para seguimiento de errores/contexto
      ...
      try {
        if (!router.db.get("usermetrics").value()) {
          router.db.set("usermetrics", []).write();
        }
      } catch (e) {
        // Si la ruta no existe, inicializar
        router.db.set("usermetrics", []).write();
      }

      // Persistir
      router.db.get("usermetrics").push(metric).write();

      return res.status(201).json({ success: true, data: metric });
    } catch (error) {
      console.error("Error en endpoint /api/v1/usermetric:", error);
      return res
        .status(500)
  ...
  // Endpoint para recibir feedback desde AI Factory
  server.post("/api/v1/feedback", (req, res) => {
    try {
      const payload = req.body || {};

      // Campos requeridos según AIFactoryFeedbackPayload
      const required = [
        "WorkflowName",
        "Feedback",
        "QueryId",
      ...
      // Validar Feedback
      const allowedFeedback = ["good", "bad", "feedback provided"];
      if (!allowedFeedback.includes(payload.Feedback)) {
        return res.status(400).json({
          success: false,
          error: `Invalid Feedback. Allowed: ${allowedFeedback.join(", ")}`,
        });
      }
      ...
      // Construir objeto a almacenar (normalizar campos y mantener arrays)
      const entry = {
        id: uuidv6(),
        workflowName: payload.WorkflowName,
        ...
        additionalDetails: Array.isArray(payload.AdditionalDetails)
          ? payload.AdditionalDetails
          : payload.AdditionalDetails
          ? [payload.AdditionalDetails]
          : [],
        ownerDLs: Array.isArray(payload.OwnerDLs)
          ? payload.OwnerDLs
          : payload.OwnerDLs
          ? [payload.OwnerDLs]
          : [],
        userConsent: Boolean(payload.UserConsent),
        username: payload.Username,
        environment: payload.Environment,
        createdAt: new Date().toISOString(),
      };

      // Asegurarse que la colección 'feedback' exista en la DB
      try {
        if (!router.db.get("feedback").value()) {
          router.db.set("feedback", []).write();
        }
      } catch (e) {
        router.db.set("feedback", []).write();
      }
      ...
      // Persistir
      router.db.get("feedback").push(entry).write();

      return res.status(201).json({ success: true, data: entry });
    } catch (error) {
      console.error("Error en endpoint /api/v1/feedback:", error);
      return res
        .status(500)
        .json({ success: false, error: "Internal server error" });
    }
