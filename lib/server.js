    ...
    try {
      ...
      if (startDateParam || endDateParam) {
        ...
        if (isMonthlyChart) {
          ...
          filteredChart.content += ` Periodo: ${MONTHS[startIndex]}-${MONTHS[endIndex]} (solo meses seleccionados).`;
        } else {
          ...
          filteredChart.content += ` Ajustado por periodo (sin eje mensual): -10%.`;
        }
      } else {
        // Sin filtros de fecha: simular disminución general
        const decreaseFactor = 0.9; // -10%
        applyFactorToChartData(filteredChart, decreaseFactor);
        filteredChart.content += ` Simulación sin fecha: -10% en valores.`;
      }
      ...
      if (region) {
        ...
        filteredChart.content += ` Ajuste regional (${region}): reducción aplicada.`;
      }
