...
\`\`\``;
  ...
  // Endpoint para obtener datos de chart aleatorios
  server.get("/auxiliary/chart/:chartId", (req, res) => {
    try {
      // Seleccionar un chart aleatorio
      const randomIndex = Math.floor(Math.random() * chartExamples.length);
      const randomChart = chartExamples[randomIndex];
      ...
      console.error("Error in auxiliary/chart endpoint:", error);
      res.status(500).json({
        success: false,
